# Completed issues

Issues move here from [`issues.md`](issues.md) once their acceptance criteria
are met. Detail files stay in `issues/`.

Work completed before this tracker existed is not listed here; see
`docs/audits/` for the point-in-time reviews and `git log` for the history.

- [x] [Remove end-to-end encrypted DMs](issues/remove-e2ee-direct-messages.md)
- [x] [Authorize inbox targeting before enqueueing actor discovery](issues/authorize-inbox-targeting-before-actor-discovery.md)
- [x] [Define an ingestion error taxonomy with explicit Oban outcomes](issues/ingestion-error-taxonomy.md)
- [x] [Fix the precommit toolchain gate](issues/fix-precommit-toolchain-gate.md)
