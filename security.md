...
- [ ] **Bind HTTP signature `keyId` to ActivityPub `actor`**: reject inbox requests where the verified signing actor does not match the activity `"actor"` (or inline `"actor": {"id": ...}`).
